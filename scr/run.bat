...
rem start cmd /c "qemu -device ahci,id=ahci0 -drive if=none,file=build/drive.img,format=raw,id=disk0 -device ide-hd,drive=disk0,bus=ahci0.0 -fda build/osimg.bin -boot c"

start "" cmd /c qemu ^
-device piix3-ide,id=ide ^
-drive  id=disk0,file=build/drive.img,format=raw,if=none ^
-device ide-hd,drive=disk0,bus=ide.0 ^
-fda build/osimg.bin -boot c
